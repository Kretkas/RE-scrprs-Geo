...
SEEN_IDS_FILE = "seen_myhome_ids.txt"
...
def main():
    ...
    for page_num in range(1, 10):
        ...
        try:
            ...
            for item in listings:
                ...
                try:
                    item_date = datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S').replace(tzinfo=timezone.utc)
    ...
    if total == 0:
        _, n429 = send_telegram_message("🔍 За последние 24 часа новых квартир по вашему фильтру на MyHome не найдено.")
        count_429 += n429
    else:
        success, n429 = send_telegram_message(f"🟢 <b>Новые квартиры с MYHOME</b>\nЗа последние 24 часа найдено: <b>{total}</b>")
        count_429 += n429
        time.sleep(2)
        ...
        for i, item in enumerate(results):
            ...
            success, n429 = send_telegram_media_group(item['caption'], item['images'])
            count_429 += n429
            if success:
    ...
    stats_line = f"MyHome:{total}:{count_429}\n"
