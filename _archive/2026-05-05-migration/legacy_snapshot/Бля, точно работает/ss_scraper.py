...
SEEN_IDS_FILE = "realtor_scrapers/seen_ss_ids.txt"
...
def main():
    ...
    for page_num in range(1, 10):
        ...
        try:
            ...
            for item in items:
                ...
                try: item_date = datetime.strptime(date_str, '%Y-%m-%d %H:%M:%S')
    ...
    if total == 0:
        send_telegram_message("🔍 За последние 24 часа новых квартир на SS.ge не найдено.")
    else:
        send_telegram_message(f"🩷 <b>Новые квартиры с SS.GE</b>\nЗа последние 24 часа найдено: <b>{total}</b>")
        time.sleep(2)
        ...
        for i, (item, image_urls) in enumerate(collected):
            ...
            if send_telegram_media_group(item['caption'], image_urls):
